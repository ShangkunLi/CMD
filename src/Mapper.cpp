                                                               ...
                                                               DFG *t_dfg, int t_II, DFGNode *t_dfgNode, list<map<CGRANode *, int> *> *t_paths)
{
       ...
       path != t_paths->end(); ++path)
  {
    ...
    if (t_dfgNode->getPredNodes()->size() > 0)
    {
      ...
      {
        if (predDFGNode->getSuccNodes()->size() > 2 and m_mapping.find(predDFGNode) != m_mapping.end())
        {
                                            ...
                                            int t_II, DFGNode *t_dfgNode, CGRANode *t_fu)
{
  ...
  // Calculate the predecessors of the 't_dfgNode'.
  list<DFGNode *> *predNodes = t_dfgNode->getPredNodes();
  int latest = -1;
  bool isAnyPredDFGNodeMapped = false;

  cout << "...number of " << t_dfgNode->getID() << "\'s predecessors: " << predNodes->size() << endl;
  cout << "...predecessors: ";
  ...
  for (DFGNode *pre : *predNodes)
  {
    ...
    if (m_mapping.find(pre) != m_mapping.end())
    {
      ...
      // the mapped 'CGRANode' of the 'pre' and the target 'fu' of the "t_dfgNode".
...
// Try to schedule t_dfgNode based on t_path.
                      ...
                      DFGNode *t_dfgNode, map<CGRANode *, int> *t_path, bool t_isStaticElasticCGRA)
{
  ...
  // Begin from the last(dst) CGRA node on the path.
  map<int, CGRANode *>::reverse_iterator riter = reorderPath->rbegin();
  bool generatedOut = true;

  cout << "Path to CGRA Node " << riter->second->getID() << "is : ";
       ...
       iter != reorderPath->end(); ++iter)
  {
    if (iter != reorderPath->begin())
    {
      ...
      bool isBypass = false;
      // Get the duration from previous CGRA node to the current one.
      int duration = (t_II + ((*iter).first - (*previousIter).first) % t_II) % t_II;
      // If current CGRA node is not dst CGRA node. And current CGRA node is the next CGRA node of previous one
  ...
  cout << "Pred nodes of " << t_dfgNode->getID() << " are: ";
  ...
  for (DFGNode *node : *t_dfgNode->getSuccNodes())
  {
    if (m_mapping.find(node) != m_mapping.end())
    {
      ...
      if (node->shareSameCycle(t_dfgNode) and
          node->isCritical() and t_dfgNode->isCritical())
      { // getCycleID() != -1 and
                          ...
                          bool t_isStaticElasticCGRA)
{
  ...
  // int genjsonBoundary = t_cgra->getFUCount();
  // if (genjsonBoundary < 2 * t_II)
  // {
  //   genjsonBoundary = 2 * t_II;
  // }
  // if (t_isStaticElasticCGRA)
  //   genjsonBoundary = t_dfg->getNodeCount();
  ...
  for (int i = 0; i < t_cgra->getRows(); ++i)
  {
    for (int j = 0; j < t_cgra->getColumns(); ++j)
    {
      ...
      if (targetDFGNode != NULL)
      {
        ...
        for (CGRALink *il : *inLinks)
        {
          if (il->isOccupied(0, t_II, t_isStaticElasticCGRA) and !il->isBypass(0))
          {
        ...
        {
          if (ir->isOccupied(0, t_II, t_isStaticElasticCGRA) and ir->getMappedDFGNode(0) == targetDFGNode)
          {
      ...
      for (CGRALink *il : *inLinks)
      {
        if (il->isOccupied(0, t_II, t_isStaticElasticCGRA) and il->isBypass(0))
        {
          ...
          for (CGRALink *ir : *outLinks)
          {
            if (ir->isOccupied(0, t_II, t_isStaticElasticCGRA) and ir->getMappedDFGNode(0) == bpsDFGNode)
            {
...
void Mapper::generateJSON4IncrementalMap(CGRA *t_cgra, DFG *t_dfg)
{
  ...
  for (int i = 0; i < t_cgra->getRows(); ++i)
  {
    for (int j = 0; j < t_cgra->getColumns(); ++j)
    {
      ...
      if (currentCGRANode->getInLinks() == 0)
        continue;
  ...
  for (int level = 0; level < FanIOs.size(); level++)
  {
    ...
    for (auto tile : tiles)
    {
      idx += 1;
      if (idx < numTiles)
        jsonFile << "          \"" << tile->getID() << "\":" << level << "," << endl;
      else
        jsonFile << "             \"" << tile->getID() << "\":" << level << endl;
    }
  }
...
{
  if (readRefMapRes(t_cgra, t_dfg) == -1)
    return -1;
